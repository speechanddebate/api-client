...
import { getMemberTabroomHistory } from './memberTabroomHistory';

describe('getMemberTabroomHistory', () => {
    it('should fetch successfully', (done) => {
        const scope = nock(new RegExp(config.API_HOST))
        .get(/tabroom/)
        .reply(200, JSON.stringify([{ test: 'test' }]));

        getMemberTabroomHistory().then((response) => {
    ...
    it('should retry on failure', () => {
        ...
        return getMemberTabroomHistory().then((response) => {
